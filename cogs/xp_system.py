...
BAR_CELLS = 17    # exact width requested
BAR_FILLED = "▰"
BAR_EMPTY  = "▱"

# Zephyra rank emotes (1..3) — provided by you
...
PAGE_SIZE = 10  # leaderboard page size
...
def bar_line(cur: int, nxt: int) -> str:
    """Single-line bar using ▰/▱ of fixed width BAR_CELLS."""
    if nxt <= 0:
...
def rank_emoji(i: int) -> str:
    if i == 1: return EMO_R1
    if i == 2: return EMO_R2
    if i == 3: return EMO_R3
    return f"#{i}"
...
class XpSystem(commands.Cog):
    ...
    # allow other cogs to ping us after adding XP
    ...
    async def leaderboard(self, interaction: discord.Interaction, page: int = 1):
        ...
        for idx, (user_id, xp, msgs, trans, vsec) in enumerate(rows, start=start_rank):
            badge = rank_emoji(idx if idx <= 3 else 0) if idx <= 3 else f"#{idx}"
            member = interaction.guild.get_member(user_id)
            name = member.display_name if member else f"<@{user_id}>"
            lvl, cur, nxt, pct = level_stats(xp)
            lines.append(
                f"{badge} **{name}** — L{lvl} • {xp} XP • msgs: {msgs} • trans: {trans} • voice: {voice_fmt(vsec)}"
            )
